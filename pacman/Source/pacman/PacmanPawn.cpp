...
void APacmanPawn::SetDirection(const FVector Direction)
{
	...
	if (Direction == FVector::DownVector) SetActorRotation(FRotator(0,90,0));
	if (Direction == FVector::RightVector) SetActorRotation(FRotator(0,0,0));
	if (Direction == FVector::LeftVector) SetActorRotation(FRotator(0,180,180));
}

void APacmanPawn::OnOverlapBegin(AActor* PlayerActor, AActor* OtherActor)
{
	if (OtherActor->ActorHasTag("FoodieRegular") || OtherActor->ActorHasTag("FoodiePowerUp"))
	{
